...
shaftWeightLength = 5; // the thickness of the weight
...
wireLockBlockOffsetY = 30; // from center
...
penHolderRadius = 5;
penHolderThick = 1.5;
penHolderOffset = 10; // depth of the pen holder
...
module pivotArm(){
	difference(){
		union(){
			...
			translate([-(pivotArmThick/2),pivotArmThick,0]) cube([ pivotArmThick, pivotArmLegLength, pivotArmLegHeight ]);
		}
		// mount hole
		...
		// pivot hole
		translate([ (pivotArmThick/2), pivotArmLegLength-(pivotArmLegHeight/2), (pivotArmLegHeight/2) ]) rotate([ 0, 0, 90 ]) trapNutDie(channelHeight=0 ,holeLength=15, holeOffset=0, nutOffset=0, nutHeight=pivotArmThick );
	}
}
	...
	difference(){
		union(){
			translate([penHolderOffset,0,0]) difference() {
				cylinder(r=penHolderRadius+penHolderThick, h=pivotArmLegHeight);
				translate([0,0,-1]) cylinder(r=penHolderRadius, h=pivotArmLegHeight+2);
